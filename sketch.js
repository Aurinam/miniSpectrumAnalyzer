...
//scaling parameters
let varScale_t = 80;	//apm inc
let horizScale_t = 1;	//expands

function time_domain_graph(p){
	p.setup = function () {
		...
		fetch('/ip_signal')
			...
			.then((pts) => {
				...
				let prevPt = pts[0]['xt'] * varScale_t;
				let prevx = pts[0]['t']*horizScale_t; 
				for (let i = 1; i < sizePt; i++) {
					let pt = pts[i]['xt'] * varScale_t;
					let x = pts[i]['t'] * varScale_t;
                    console.log(x, pt);
					p.line(prevx, prevPt, x, pt);
					prevPt = pt;
                    prevx = x;
				} 
			})
		...
		//title
		p.text("Time Domain Graph", -800, -800);

		p.pop();
	}
}
new p5(time_domain_graph);
